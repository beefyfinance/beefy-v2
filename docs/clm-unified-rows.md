...
| Toggle labels | Buttons name the products with their icons: "🐾 Reward Pool &#124; ⟳ Autocompound" (no separate label text); retired side gets "(Retired)" suffix; wraps to its own full-width line below the sm breakpoint | Originally "AUTOCOMPOUNDING Off/On" — revised 2026-08-16: with product naming removed from tags, the toggle is where the products get named, and the icons match the stat-line legend | — |
