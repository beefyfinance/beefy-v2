...
interface VaultLP extends VaultBase { // name it pool ?
    tokens: {
        lpToken: Token["id"]
        earnedToken: Token["id"]
        tokens: Token["id"][]
...
interface BeefyAPIBreakdownShort { vaultApr: number }
...
const fetchVaultList = createAsyncThunk<{chainId: Chain["id"], pools: Awaited<ReturnType<VaultAPI["fetchByChainId"]>>}, {chainId: Chain["id"]}>(
    'vaults/fetchVaultListForChain',
    async ({chainId}) => {
...
const tokensInitialState: NormalizedEntity<Token> = {
    allIds: [],
    byId: {}
}
            ...
            for (const vault of action.payload.pools) {
                if (state.byId[vault.earnedToken] === undefined) {
                    const token : Token = {
                        ...
                        price: new BigNumber(0), // maybe split price
...
// now we handle this action 
const vaultsInitialState: NormalizedEntity<Vault> & {
...
type LoaderState = LoaderStateInit | LoaderStatePending | LoaderStateRejected | LoaderStateFulfilled
