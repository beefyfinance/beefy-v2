...
const getSingleChainPools = async (pools, items, state, key, prices, apy, web3) => {
  const chain = config[key];
  const rpc = state.walletReducer.rpc[key];
  const multicall = new MultiCall(rpc, config[key].multicallAddress);

  const calls = [];

  // console.log(items);
  // const chainItems = items.filter(item => item.network === key);

  for (let itemKey in items) {
    const pool = items[itemKey];
    if (pool.network !== key) continue;
    if (pool.isGovVault) {
  ...
  const promises = [];
  promises.push(multicall.all([calls]));
...
const getPools = async (items, state, dispatch) => {
  ...
  let promises = [];
  for (let key in web3) {
    promises.push(getSingleChainPools(pools, items, state, key, prices, apy, web3));
    // multicall[key] = new MultiCall(web3[key], config[key].multicallAddress);
    // calls[key] = [];
  }
  let res = await Promise.all(promises);
  let totalTvl = res.reduce((tot, cur) => tot.plus(cur));
...
const getSingleChainBoosts = async (boosts, pools, items, state, key, web3, prices) => {
  const multicall = new MultiCall(web3[key], config[key].multicallAddress);
  const calls = [];
  const moos = [];

  for (let boostKey in items) {
    const pool = items[boostKey];
    if (pool.network !== key) continue;
  ...
  const promises = [];
  promises.push(multicall[key].all([calls]));
  promises.push(multicall[key].all([moos]));
...
const getBoosts = async (items, state, dispatch) => {
  ...
  for (let key in web3) {
    promises.push(getSingleChainBoosts(boosts,pools,items,state,key,web3,prices));
    // multicall[key] = new MultiCall(web3[key], config[key].multicallAddress);
    // calls[key] = [];
    // moos[key] = [];
  }
