        ...
        {isLoading ? (
            ...
            <Grid container style={{position: 'relative'}}>
                <Grid item xs={12} md={8} lg={8} xl={9}>
                    <Button className={classes.btnGoBack} 
                                            onClick={() => {history.push('/')}}>
                        <ArrowLeft /> {t( 'Vault-GoBack')}
                    </Button>
                    <Grid className={classes.title} container>
                        <Grid>
                            <AssetsImage img={item.logo} assets={item.assets} 
                                                                    alt={item.name}/>
                        </Grid>
                        <Grid>
                    ...
                    <Box className={classes.mobileFix} display="flex" alignItems="center">
                        <Box display={"flex"} alignItems="center">
                            ...
                            <Box pl={1}>
                                <Typography className={classes.network} display={"inline"}>
                                    {item.network} {t( 'Vault-network')}</Typography>
                            </Box>
                            <Box pl={1}>
                        ...
                        <Box className={classes.summaryContainer} display={"flex"} 
                            ...
                            <Box>
                                <Typography variant={"h1"}>
                                    {calcDaily(item.apy.totalApy)}</Typography>
                                <Typography variant={"body2"}>
                                    {t( 'Vault-Daily')}</Typography>
                            </Box>
                            <Box>
                                <Divider />
                            </Box>
                            <Box>
                                <Typography variant={"h1"}>
                                    {formatApy(item.apy.totalApy)}</Typography>
