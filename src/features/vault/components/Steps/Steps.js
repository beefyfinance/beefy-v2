        ...
        ><Fade in={steps.modal}>
            ...
            <React.Fragment><Box>
                <Box p={8} className={classes.finishedCard}>
                    {steps.items[steps.currentStep].step === 'deposit' ? (
                        <React.Fragment>
                            <Typography variant={"h2"}>
                                {byDecimals(new BigNumber(wallet.action.data.amount), item.tokenDecimals).toFixed(8)} {item.token}
                            </Typography>
                        ...
                        <React.Fragment>
                            <Typography variant={"h2"}>
                                {byDecimals(new BigNumber(wallet.action.data.amount).multipliedBy(byDecimals(item.pricePerFullShare)), item.tokenDecimals).toFixed(8)} {item.token}
                            </Typography>
                    ...
                    <Box mt={1} textAlign={"center"}>
                        <Button className={classes.finishedBtn} href={wallet.explorer[item.network] + '/tx/' + wallet.action.data.receipt.transactionHash} target="_blank">
                            {t( 'Transactn-View')}
                        </Button> <Button className={classes.finishedBtn} onClick={handleClose}>
                            {t( 'Transactn-CloseDialog')}
                        </Button>
                    </Box>
                </Box>
                <Box mt={2} textAlign={"center"}>
                    <Button onClick={() => {history.push({pathname: '/', portfolioOpen: true})}} className={classes.finishedBtn}>
            ...
            <Box className={classes.paper}>
                <Typography id="transition-modal-title" variant={"h2"}>
                    {steps.currentStep} / {steps.items.length}<br/>{t( 'Transactn-Confirmed')}
                </Typography>
                ...
                {wallet.action && wallet.action.result === 'success_pending' ? (
                    <Alert severity={"info"}>
                        ...
                        <Box textAlign={"center"} mt={2}>
                            <Button variant={"outlined"} href={wallet.explorer[item.network] + '/tx/' + wallet.action.data.hash} target="_blank">
                                {t( 'Transactn-View')}
                            </Button>
                        </Box>
                    </Alert>
