...
describe('scoreVaultForSearch tiers', () => {
  ...
  it('60/50: stock name words match whole (60) or by 3+ char prefix (50), never by substring', () => {
    ...
    const apple = makeVault({ name: 'AAPL-USDG' });
    const appleWords = toSearchWords('Apple');
    expect(scoreVaultForSearch(context('apple'), apple, ['AAPL', 'USDG'], appleWords, [])).toBe(60);
    expect(scoreVaultForSearch(context('appl'), apple, ['AAPL', 'USDG'], appleWords, [])).toBe(50);
    expect(scoreVaultForSearch(context('ro'), apple, ['AAPL', 'USDG'], appleWords, [])).toBe(0);
    expect(
      scoreVaultForSearch(context('apple usdg'), apple, ['AAPL', 'USDG'], appleWords, [])
    ).toBe(60);
    expect(scoreVaultForSearch(context('nvidia'), apple, ['AAPL', 'USDG'], appleWords, [])).toBe(0);
    const spy = makeVault({ name: 'SPY-USDG' });
    const spyWords = toSearchWords('SPDR S&P 500 ETF Trust');
    expect(scoreVaultForSearch(context('s&p 500'), spy, ['SPY', 'USDG'], spyWords, [])).toBe(60);
    expect(scoreVaultForSearch(context('spdr'), spy, ['SPY', 'USDG'], spyWords, [])).toBe(60);
    // "base" must not surface Coinbase above Base-chain vaults
    const coin = makeVault({ name: 'COIN-WETH', chainId: 'robinhood' });
    const coinWords = toSearchWords('Coinbase');
    expect(scoreVaultForSearch(context('base'), coin, ['COIN', 'WETH'], coinWords, [])).toBe(0);
  });
  ...
  it('DIVERGENCE vs old matcher: per-word token substrings match multi-word queries', () => {
    ...
    const single = makeVault({ name: 'Something Else' });
    expect(scoreVaultForSearch(context('steth btcb'), single, ['wstETH', 'aBTCb'], [], [])).toBe(
      50
    );
  });
