...
function handleCLMsTimeline(timeline: CLMTimelineAnalyticsEntity[], state: BeefyState) {
  ...
  return groupBy(vaultTxs, tx => {
    const vaultAddress = tx.productKey.replace(`beefy:vault:${tx.chain}:`, '');

    const vaultId =
      state.entities.vaults.byChainId[tx.chain]?.cowcentratedVault.byEarnedTokenAddress[
        vaultAddress
      ];

    return vaultId || tx.displayName;
  });
}
