...
export const boostSlice = createSlice({
  ...
  reducers: {
    ...
    setMax(
      ...
      const balanceToken = selectTokenByAddress(state, vault.chainId, vault.earnContractAddress);
      const balance =
        sliceState.mode === 'stake'
          ? selectUserBalanceOfToken(state, vault.chainId, vault.earnContractAddress) // mootoken
