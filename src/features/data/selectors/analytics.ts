...
import { ClmPnl, mergeClmPnl, PnL } from '../../../helpers/pnl.ts';
...
const selectStandardVaultPnlModel = createCachedSelector(
...
export const selectStandardGovPnl = createCachedSelector(
  [
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress?: string) =>
      selectVaultById(state, vaultId),
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress?: string) =>
      selectStandardVaultPnlModel(state, vaultId, walletAddress),
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress?: string) => {
    ...
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress?: string) =>
      selectUserVaultBalanceInShareTokenIncludingDisplaced(state, vaultId, walletAddress),
  ],
  (
    vault,
    pnl,
    ...
    const ppfs = ppfsRaw.shiftedBy(18 - depositToken.decimals);
...
const selectClmVaultPnlModel = createCachedSelector(
  (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
    selectUserDepositedTimelineByVaultId(state, vaultId, walletAddress),
  (sortedTimeline): ClmPnl => {
...
const selectClmPnlTokens = createCachedSelector(
  ...
  (state: BeefyState, vaultId: VaultEntity['id']) => {
    const vault = selectCowcentratedLikeVaultById(state, vaultId);
    return selectTokenByAddress(
      state,
      vault.chainId,
      getCowcentratedAddressFromCowcentratedLikeVault(vault)
    );
  },
  (depositTokens, shareToken, underlyingToken) => ({
    token0: depositTokens[0],
    token1: depositTokens[1],
    shareToken,
    underlyingToken,
  })
)((_state: BeefyState, vaultId: VaultEntity['id']) => vaultId);

const selectClmPnlLivePrices = createCachedSelector(
  (state: BeefyState, vaultId: VaultEntity['id']) => {
    const vault = selectCowcentratedLikeVaultById(state, vaultId);
    return selectTokenPriceByAddress(state, vault.chainId, vault.depositTokenAddress);
  },
  (state: BeefyState, vaultId: VaultEntity['id']) => {
    const [token0] = selectCowcentratedLikeVaultDepositTokens(state, vaultId);
    return selectTokenPriceByAddress(state, token0.chainId, token0.address);
  },
  (state: BeefyState, vaultId: VaultEntity['id']) => {
    const [, token1] = selectCowcentratedLikeVaultDepositTokens(state, vaultId);
    return selectTokenPriceByAddress(state, token1.chainId, token1.address);
  },
...
const selectTokenAndPriceSlices = createSelector(
  (state: BeefyState) => state.entities.tokens.byChainId,
  (state: BeefyState) => state.entities.tokens.prices.byOracleId,
  (byChainId, byOracleId) => ({ byChainId, byOracleId })
);

const selectClmPnlInner = createCachedSelector(
  [
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress: string) =>
      selectCowcentratedLikeVaultById(state, vaultId),
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
      selectClmVaultPnlModel(state, vaultId, walletAddress),
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress: string) =>
      selectClmPnlTokens(state, vaultId),
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
      selectUserVaultBalanceInShareTokenIncludingDisplaced(state, vaultId, walletAddress),
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress: string) =>
      selectClmPnlLivePrices(state, vaultId),
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress: string) => {
      const vault = selectCowcentratedLikeVaultById(state, vaultId);
      return selectLpBreakdownForVault(state, vault);
    },
    (state: BeefyState, vaultId: VaultEntity['id'], _walletAddress: string) =>
      selectVaultPricePerFullShare(state, vaultId),
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
    ...
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
      selectGovVaultPendingRewardsWithPrice(state, vaultId, walletAddress),
    (state: BeefyState, vaultId: VaultEntity['id'], walletAddress: string) =>
      selectClmPnlOffChainRewards(state, vaultId, walletAddress),
    (state: BeefyState, _vaultId: VaultEntity['id'], _walletAddress: string) =>
      selectTokenAndPriceSlices(state),
  ],
  (
    vault,
    clmPnl,
    ...
    const { token0, token1, shareToken, underlyingToken } = tokens;
    const {
      underlying: liveUnderlyingPrice,
      token0: liveToken0Price,
      token1: liveToken1Price,
    } = livePrices;
    const { merkl: merklRewards, stella: stellaSwapRewards } = offChainRewardsData;
    const { byChainId: tokensByChainId, byOracleId: pricesByOracleId } = slices;

    // === Now (Live, on-chain)
    const liveTotalSupply = new BigNumber(liveBreakdown.totalSupply);
    const livePpfs = livePpfsRaw.shiftedBy(18 - underlyingToken.decimals);
    ...
    for (const [tokenAddress, entry] of Object.entries(clmPnl.getClaimed().tokens)) {
      const token = valueOrThrow(
        tokensByChainId[vault.chainId]?.byAddress[tokenAddress.toLowerCase()],
        `selectTokenByAddress: Unknown token address "${tokenAddress}"`
      );
...
export const selectClmPnl = (
  ...
  const resolvedWalletAddress = walletAddress ?? selectWalletAddress(state);
 ...
 * single side for anything that is not a merged CLM.
 */
export const selectClmGroupPnl = (
  state: BeefyState,
  vaultId: VaultEntity['id'],
  walletAddress?: string
): UserClmGroupPnl => {
  const address = walletAddress ?? selectWalletAddress(state);
  if (!address) {
    throw new Error('No wallet address provided');
  }
 ...
 * same two tokens, attributed by that side's balance, so amounts and USD sum. Harvests are
 * concatenated in time order and re-cumulated, since each side's running totals only counted itself.
 */
export function mergeClmHarvestTimelines(
  timelines: ClmUserHarvestsTimeline[]
): ClmUserHarvestsTimeline {
  const present = timelines.filter(t => t.harvests.length > 0);
  if (present.length === 0) {
    return timelines[0];
  }
  if (present.length === 1) {
  ...
  const tokens = present[0].tokens;
  const sumPerToken = (pick: (t: ClmUserHarvestsTimeline) => BigNumber[]) =>
    tokens.map((_, i) => present.reduce((acc, t) => acc.plus(pick(t)[i] ?? BIG_ZERO), BIG_ZERO));

  const cumulativeAmounts = tokens.map(() => BIG_ZERO);
  const cumulativeAmountsUsd = tokens.map(() => BIG_ZERO);
  let cumulativeTotalUsd = BIG_ZERO;

  const harvests = present
    .flatMap(t => t.harvests)
    .sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime())
    .map(harvest => {
      harvest.amounts.forEach((amount, i) => {
        cumulativeAmounts[i] = cumulativeAmounts[i].plus(amount);
        cumulativeAmountsUsd[i] = cumulativeAmountsUsd[i].plus(harvest.amountsUsd[i]);
      });
      cumulativeTotalUsd = cumulativeTotalUsd.plus(harvest.totalUsd);
      return {
        ...harvest,
        cumulativeAmounts: [...cumulativeAmounts],
        cumulativeAmountsUsd: [...cumulativeAmountsUsd],
        cumulativeTotalUsd,
      };
    });
  ...
  return {
    tokens,
    harvests,
    totals: sumPerToken(t => t.totals),
    totalsUsd: sumPerToken(t => t.totalsUsd),
    totalUsd: present.reduce((acc, t) => acc.plus(t.totalUsd), BIG_ZERO),
  };
}
