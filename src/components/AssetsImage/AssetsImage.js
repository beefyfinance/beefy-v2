...
const AssetsImage = ({img, assets, alt}) => {
    ...
    const singleImage = img ? require('../../images/' + img).default
        : assets.length === 1 ? getSingleAssetSrc(assets[0]) : undefined

    return (
        singleImage ? (
            <Avatar className={classes.large} alt={alt} src={singleImage}
                    variant="square"
