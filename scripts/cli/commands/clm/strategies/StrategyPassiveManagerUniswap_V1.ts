...
import { parseVaultName } from '../vault';
import { getPlatforms } from '../../../lib/config/platform';
import { AddressBookChainId } from '../../../../common/config';

const OKU_CHAINS: AddressBookChainId[] = ['mantle', 'linea', 'manta'];

export default {
  tokenProviderId: async (chainId, strategyAddress, vaultReceiptName) => {
    if (OKU_CHAINS.includes(chainId)) {
      return 'oku';
    }

    const name = parseVaultName(vaultReceiptName);
    const allPlatforms = await getPlatforms();
    const idMatching = allPlatforms.find(
      platform => platform.id.toLowerCase() === name.platform.toLowerCase()
    );
    if (idMatching) {
      return idMatching.id;
    }
    const nameMatching = allPlatforms.find(
      platform => platform.name.toLowerCase() === name.platform.toLowerCase()
    );
    if (nameMatching) {
      return nameMatching.id;
    }
  ...
  abi: [
    ...
    },
    {
      anonymous: false,
      inputs: [
        { indexed: false, internalType: 'uint256', name: 'feeMain0', type: 'uint256' },
        {
          indexed: false,
          internalType: 'uint256',
          name: 'feeMain1',
        ...
        { indexed: false, internalType: 'uint256', name: 'feeAlt0', type: 'uint256' },
        {
          indexed: false,
          internalType: 'uint256',
          name: 'feeAlt1',
    ...
    },
    {
      ...
      name: 'getKeys',
      outputs: [
        { internalType: 'bytes32', name: 'keyMain', type: 'bytes32' },
        {
