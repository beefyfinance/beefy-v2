...
type Hyphenate<T extends string> = T extends `${infer Head}${infer Tail}`
  ? `${Head extends Capitalize<Head> ? '-' : ''}${Lowercase<Head>}${Hyphenate<Tail>}`
  : Lowercase<T>;

type TransformPoolType<T extends string> = T extends `${infer First}${infer Rest}`
  ? Hyphenate<`${Lowercase<First>}${Rest}`>
  : Hyphenate<T>;
...
async function start() {
  ...
  await saveZap(chain, args.vault, zap);
}
