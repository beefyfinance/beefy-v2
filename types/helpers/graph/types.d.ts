...
export type RechartsTooltipProps<KValue extends string, KName extends string, TPayload extends {
    [key in KValue]: ValueType;
} & {
    [key in KName]: NameType;
}, TValue extends ValueType = TPayload[KValue], TName extends NameType = TPayload[KName]> = Omit<TooltipProps<TValue, TName>, 'payload'> & {
    payload?: Array<Omit<Payload<TValue, TName>, 'payload'> & {
        payload?: TPayload;
    }>;
};
