...
export type SnakeToCamelCase<Key extends string> = Key extends `${infer FirstPart}_${infer FirstLetter}${infer LastPart}` ? `${FirstPart}${Uppercase<FirstLetter>}${SnakeToCamelCase<LastPart>}` : Key;
...
export type Rest<TPicked, TFull extends Record<keyof TPicked, unknown>> = TPicked & Omit<TFull, keyof TPicked>;
export type MapNullToUndefined<T extends object> = {
    [K in keyof T]: NullToUndefined<T[K]>;
};
type Web3KeepTypes = boolean;
type Web3ConvertType<T> = T extends Array<infer U> ? Web3ConvertType<U>[] : T extends Web3KeepTypes ? T : string;
...
export type EnsureKeys<TKey extends string, TObj extends {
    [key in TKey]: unknown;
}> = {
...
export type FCWithRef<T, E> = FC<T & {
    ref: ForwardedRef<E>;
}>;
